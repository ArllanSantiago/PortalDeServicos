...
export class ContasLevantadasComponent extends ContasComponent {
    constructor( varaService: VaraService) {
        super();
        this.contasView.title = "Contas Levantadas"
        this.contasView.grid = {
            data: new ResponsePagination, columns: [
                { field: '', headerName: '', filter: false, resizable: true, sortable: true, width: 100 }
            ]
        }
    }
    ...
    gerarText(builder: any, p_orientation: 'l' | 'p'): jsPDF {
        ...
        if (this.contasView.controls['numSeqIni']) {
            ...
            doc.text(` de ${this.contasView.controls['numSeqIni']} até ${this.contasView.controls['numSeqIni']}`, 44, proxlinha, { align: alginText })
        }
