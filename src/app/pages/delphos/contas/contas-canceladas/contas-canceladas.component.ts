...
export class ContasCanceladasComponent extends ContasComponent {
    constructor(  private contasCancelService:ContasCanceladasService
        ){
        super();
        this.contasView.title = "Contas Canceladas";
        this.contasView.subTitle = "Consulta as Contas Canceladas";
            ...
            utils.loading(
                        ...
                        if (!res.empty) {
                            ...
                            _content = res.content.map(function (item:any) {
                                ...
                                return {
                                    ...
                                    , valorDepositoCurrency: utils.formatterCurrency.format(utils.correctCurrency(item.valorDeDeposito))
                                }
                            })
                        }
                        //carregando Grid
                        this.contasView.grid = {
                            ...this.contasView.grid, data: { ...res, content: _content }
                        }
                ...
                if (this.contasView.dataRel?.length == 0) {
                    ...
                    this.contasView.pdf = this.gerarText({
                        ...
                        , nameRel: `Relatório de ${this.contasView.title}`
                    }, 'l')
                }
            })
        } else {
    ...
    gerarText(builder: any, p_orientation:string): jsPDF {
        ...
        if (this.contasView.controls['documento']) {
        ...
        if (this.contasView.controls['dataIni']) {
        ...
        if (this.contasView.controls['numSeqIni']) {
        ...
        if (this.contasView.controls['acaoOriginaria']) {
        ...
        if (this.contasView.controls['acaoExecutoria']) {
            doc.text(`Número da Ação Executória: `, INI_PARAGRAF, proxlinha += DIF_BETWEEN_LINE, { align: alginText })
            doc.text(` ${this.contasView.controls['acaoExecutoria']}`, 69, proxlinha, { align: alginText })
        }
        ...
        doc.text(`${this.contasView.controls['ordenacao'].value}`, 29, proxlinha, { align: alginText })
        ...
        builder.data.forEach((dLine :any) => {
            ...
            if (builder.data.indexOf(dLine) == 0) {
                ...
                configPdf.dimensionsWField.push(_INI_PARAGRAF += (DIF_HEADER * 2) + Math.round(doc.getTextDimensions(dLine[field[5]]).w))
                //
                _INI_PARAGRAF = INI_PARAGRAF
                configPdf.dimensionsWField.push(_INI_PARAGRAF)
            ...
            if (dLine) {
                ...
                doc.text(String(dLine[field[7]]), configPdf.dimensionsWField[7], proxlinha += (DIF_BETWEEN_LINE * 1.3), { align: 'left' })
